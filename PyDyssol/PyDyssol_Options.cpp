...
//Options
py::dict PyDyssol::GetOptions() const
{
    py::dict out;
...
void PyDyssol::SetOptions(const py::dict& options)
{
    ...
    auto get_double = [&](const char* key, auto setter) {
        if (options.contains(key)) {
            double val = options[key].cast<double>();
            (p->*setter)(val);
        }
        };
    auto get_uint = [&](const char* key, auto setter) {
        if (options.contains(key)) {
            uint32_t val = options[key].cast<uint32_t>();
            (p->*setter)(val);
        }
        };
    auto get_bool = [&](const char* key, auto setter) {
        if (options.contains(key)) {
            bool val = options[key].cast<bool>();
            (p->*setter)(val);
        }
        };
    auto get_enum_conv = [&](const char* key) {
        if (options.contains(key)) {
            std::string val = options[key].cast<std::string>();
...
py::dict PyDyssol::GetOptionsMethods() const
{
    py::dict result;
