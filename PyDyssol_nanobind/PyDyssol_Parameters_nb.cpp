...
nanobind::object GetNativeUnitParameter(const CBaseUnitParameter* param, const CMaterialsDatabase& db)
{
    ...
    case EUnitParameter::MDB_COMPOUND:
        if (const auto* cast = dynamic_cast<const CCompoundUnitParameter*>(param)) {
            ...
            return nb::str(comp ? comp->GetName().c_str() : cast->GetCompound().c_str());
        }
    ...
    case EUnitParameter::PARAM_DEPENDENT:
        if (const auto* dep = dynamic_cast<const CDependentUnitParameter*>(param))
            return nb::cast(dep->GetParamValuePairs());
        break;
    default:
        break;
    }
...
std::map<std::string, std::tuple<nanobind::object, std::string, std::string>> PyDyssol::GetUnitParameters(const std::string& unitName) const
{
    ...
    std::map<std::string, std::tuple<nanobind::object, std::string, std::string>> result;
    for (const auto* param : activeParams) {
        ...
        nanobind::object value = GetNativeUnitParameter(param, m_materialsDatabase);
...
std::map<std::string, std::tuple<nanobind::object, std::string, std::string>> PyDyssol::GetUnitParametersAll(const std::string& unitName) const
{
    const auto* unit = m_flowsheet.GetUnitByName(unitName);
    if (!unit)
        throw std::runtime_error("Unit not found: " + unitName);
    ...
    > value)
{
    ...
    if (std::holds_alternative<bool>(value)) {
    ...
    else if (std::holds_alternative<std::vector<nanobind::dict>>(value)) {
        ...
        const std::vector<nanobind::dict>& pyReactions = std::get<std::vector<nanobind::dict>>(value);
        std::vector<CChemicalReaction> reactions;

        for (const nanobind::dict& pyRxn : pyReactions) {
            CChemicalReaction reaction;
            reaction.SetName(nb::cast<std::string>(pyRxn["name"]));

            std::string baseSubstanceKey = nb::cast<std::string>(pyRxn["base"]);
            ...
            const auto pySubstances = nb::cast<std::vector<nanobind::dict>>(pyRxn["substances"]);
            for (const nanobind::dict& pySubstance : pySubstances) {
                CChemicalReaction::SChemicalSubstance substance;
                substance.key = nb::cast<std::string>(pySubstance["key"]);
                substance.nu = nb::cast<double>(pySubstance["nu"]);
                substance.order = nb::cast<double>(pySubstance["order"]);
                substance.phase = GetPhaseByName(nb::cast<std::string>(pySubstance["phase"]));
...
nb::object PyDyssol::GetUnitParameter(const std::string& unitName, const std::string& paramName) const
{
    const auto* unit = m_flowsheet.GetUnitByName(unitName);
    if (!unit)
        throw std::runtime_error("Unit not found: " + unitName);
